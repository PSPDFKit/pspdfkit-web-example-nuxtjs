...
In order to make the PSPDFKit for Web's library available for building, we have to copy the `pspdfkit-lib/` directory from `node_modules/pspdfkit/dist/` into the `static/js/` directory. This is done in the `package.json` `postinstall` script.
